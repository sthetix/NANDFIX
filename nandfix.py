...
class NXUnbrickerGUI:
    ...
    def __init__(self, root):
        ...
        self.nxnandmanager_path = os.path.join(self.base_path, 'bin', 'nxnandmanager')
        self.emmc_hacc_gen_path = os.path.join(self.base_path, 'bin', 'EmmcHaccGen.exe')

        print(f"Base path set to: {self.base_path}")
        print(f"Full path to nxnandmanager: {self.nxnandmanager_path}")
        print(f"Full path to EmmcHaccGen.exe: {self.emmc_hacc_gen_path}")
    ...
    def create_widgets(self):
        ...
        for i, operation in enumerate(operations):
            ttk.Button(self.root, text=operation, width=10, command=lambda op=operation: self.handle_operation(op)).place(
                relx=0.025 + (i * 0.25), rely=0.868, height=26, width=80
            )
        ...
        self.footer_label = tk.Label(self.root, text="v1.0.1 sthetix", font=("Helvetica", 10), bg=self.root["background"])
    ...
    def run_generation(self, prod_keys, firmware_folder, console_type):
        ...
        try:
            ...
            self.create_popup("Generation Error", f"An error occurred during NAND files generation: {e}", "error")
        finally:
            self.stop_timer()
    ...
    def run_decryption(self):
        ...
        try:
            ...
            self.create_popup("Decryption Error", f"An error occurred during decryption: {e}", "error")
        finally:
            self.stop_timer()
    ...
    def decrypt_nand(self, entries, update_progress):
        ...
        for i, command_args in enumerate(decryption_commands):
            ...
            if result.stderr:
                print(f"Warning/Error from nxnandmanager: {result.stderr}")
            update_progress((i + 1) / len(decryption_commands) * 100, f"Decrypting {command_args[7].split('=')[1]}...")
    ...
    def run_encryption(self):
        ...
        try:
            ...
            self.create_popup("Encryption Error", f"An error occurred during encryption: {e}", "error")
        finally:
            self.stop_timer()
    ...
    def run_build_process(self):
        ...
        try:
            ...
            self.create_popup("Build Error", f"An error occurred during the build process: {e}", "error")
        finally:
            self.stop_timer()
    ...
    def build_nand(self, entries, update_progress):
        ...
        for i, command_args in enumerate(build_commands):
            ...
            if result.stderr:
                print(f"Warning/Error from nxnandmanager: {result.stderr}")
            update_progress((i + 1) / len(build_commands) * 100, f"Building {command_args[5].split('=')[1]}...")
        ...
        print(f"Renaming {original_donor_rawnand_path} to {final_rawnand_path}")
    ...
    def create_popup(self, title, message, popup_type="info"):
        ...
        return messagebox.showinfo(title, message) if popup_type == "info" else \
               messagebox.showwarning(title, message) if popup_type == "warning" else \
               messagebox.showerror(title, message) if popup_type == "error" else \
